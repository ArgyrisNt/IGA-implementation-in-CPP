// Example of a trimmed NURBS surface
...
int main()
{
    ...
    for (int j = (int) (left_limit_y); j <= 100; j++)
    {
        for (int i = (int) (left_limit_x); i <= 100; i++)
        {
            ...
            int span_i = bspline_x.findSpanInVector(i_step);
            int span_j = bspline_y.findSpanInVector(j_step);
            std::vector<double> bVal_i = bspline_x.evaluateAtPoint(i_step).first;
            std::vector<double> bVal_j = bspline_y.evaluateAtPoint(j_step).first;
            ...
            for (int ii = 0; ii < bVal_i.size(); ii++)
			{
				for (int jj = 0; jj < bVal_j.size(); jj++)
				{
                    ...
                    coord_x += bVal_i[ii] * bVal_j[jj] * controlPoints[index][0];
                    coord_y += bVal_i[ii] * bVal_j[jj] * controlPoints[index][1];
                }
            }
