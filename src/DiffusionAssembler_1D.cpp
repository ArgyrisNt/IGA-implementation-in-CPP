...
{
	double v = 0.0;
	std::vector<std::pair<double, double>> gauss = GaussPointsAndWeightsQuad(bspline_x->getDegree() + 3, getDistinctKnotX(element), getDistinctKnotX(element + 1));
...
std::vector<double> DiffusionAssembler_1D::applyInitialCondition(double (*func)(double))
{
	...
	double start = getDistinctKnotX(0);
	double end = getDistinctKnotX(getDistinctKnotsX().size() - 1);
