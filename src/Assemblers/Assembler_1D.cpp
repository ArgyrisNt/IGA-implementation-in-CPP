...
{
	return bspline_x->getNumberOfBasisFunctions();
}
...
Matrix<double> Assembler_1D::Jacobian(double g1, std::vector<double> &Nx)
{
	...
	{
		Jacobian.setValue(0, 0, Jacobian(0, 0) + Nx[kk] * controlPoints[span_g1 - bspline_x->getDegree() + kk][0]);
		Jacobian.setValue(0, 1, Jacobian(0, 1) + Nx[kk] * controlPoints[span_g1 - bspline_x->getDegree() + kk][1]);
	}
...
{
	double v = 0.0;
	XGaussPointsAndWeights = GaussPointsAndWeightsQuad(bspline_x->getDegree() + 3, getDistinctKnotX(element), getDistinctKnotX(element + 1));
	for (int g1 = 0; g1 < XGaussPointsAndWeights.size(); g1++)
	{
		std::vector<double> gradVal = bspline_x->evaluateAtPoint(XGaussPointsAndWeights[g1].first).second;
